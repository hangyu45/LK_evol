...
fig_dir = '/home/hang.yu/public_html/astro/LK_evol/LK2merger/rand_init_spin_ang/SA/M3_%.1eMs_ao_%.3fpc_ai0_%.1fAU/'\
            %(M3, ao, ai_0)
data_dir = 'data/rand_init_spin_ang/SA/M3_%.1eMs_ao_%.3fpc_ai0_%.1fAU/'\
            %(M3, ao, ai_0)
...
M1, M2 = 85.*Ms, 65.*Ms
chi1, chi2=0.7, 0.7
Tgw_trgt = 3.e7 * P_yr # only consider a narrow range of SA for computational efficiency
...
I_io_0 = 88.5*np.pi/180.

phi_ro_0 = np.pi/4. #stats.uniform(scale=2.*np.pi).rvs()

# randomize the initial spin alignment wrt the total ang momentum
I_S1_0 = 70*np.pi/180. #stats.uniform(scale=np.pi).rvs()
I_S2_0 = 110*np.pi/180. #stats.uniform(scale=np.pi).rvs()
...
if plot_flag>0:
    ...
    ax.plot(tt/1.e3/P_yr, theta1_SL*180./np.pi, alpha=0.8, label=r'$S_1$', color='tab:grey')
    ax.plot(tt/1.e3/P_yr, theta2_SL*180./np.pi, alpha=0.8, label=r'$S_2$', color='tab:olive')
...
#     ax.plot(tt/1.e3/P_yr, theta_SS*180./np.pi, alpha=0.8, color='tab:grey')
...
# print('a/AU, 1-e, e/e_ref, t_m/P_yr', a_LK/AU, 1-e_LK, e_LK/ei[-1], '%e'%(tt[-1]/P_yr))
...
#     ax.plot(S_600/S_Mt, chi1_600, color='tab:orange', alpha=0.8, label=r'$a=600 M_{\rm t}$')
