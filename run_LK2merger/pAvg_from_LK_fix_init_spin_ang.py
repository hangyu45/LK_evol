...
fig_dir = '/home/hang.yu/public_html/astro/LK_evol/LK2merger/fix_init_spin_ang/DA/M3_%.1eMs_ao_%.3fpc_ai0_%.1fAU/'\
            %(M3, ao, ai_0)
data_dir = 'data/fix_init_spin_ang/DA/M3_%.1eMs_ao_%.3fpc_ai0_%.1fAU/'\
            %(M3, ao, ai_0)
...
for i in range(100, 100+nFile, 1):
    if os.path.exists(data_dir + 'id_%i_LK_cond.txt'%i):
        data_LK = np.loadtxt(data_dir + 'id_%i_LK_cond.txt'%i)
        
        prefix = 'id_%i_'%i # make sure the prefix is consistent 
        print('File:', i)
