...
### load input 
#######################################################################
# further reduce the inner binary's semi-major axis by 
...
br_flag, ss_flag = 1, 1
atol, rtol = 1.e-9, 1.e-9

fid=h5.File(data_dir + past_prefix + 'data.h5', 'r')
M1, M2, M3 = fid.attrs['M1'], fid.attrs['M2'], fid.attrs['M3']
mu_i=M1*M2/(M1+M2)
ao = fid.attrs['ao']
...
Lo_x_0, Lo_y_0, Lo_z_0 = fid['Lo_x'][()][-1], fid['Lo_y'][()][-1], fid['Lo_z'][()][-1]
Lo_0 = np.sqrt(Lo_x_0**2. + Lo_y_0**2. + Lo_z_0**2.)

eo_x_0, eo_y_0, eo_z_0 = fid['eo_x'][()][-1], fid['eo_y'][()][-1], fid['eo_z'][()][-1]
eo_0 = np.sqrt(eo_x_0**2. + eo_y_0**2. + eo_z_0**2.)
...
Lo_nat_v = np.array([Lo_x_0, Lo_y_0, Lo_z_0])/Lo_unit
eo_v = np.array([eo_x_0, eo_y_0, eo_z_0])
...
sol=integ.solve_ivp(int_func, \
        t_span=(t_0/t_unit, t_0/t_unit+1e4), y0=y_nat_init, rtol=rtol, atol=atol, \
        events=term_func)
#######################################################################
...
ax.semilogy(tt/1.e6/P_yr, ai/AU, alpha=0.8, color='tab:grey')
ax.set_ylabel(r'$a_{\rm in}$ [AU]')

ax=fig.add_subplot(412)
ax.semilogy(tt/1.e6/P_yr, 1.-ei, alpha=0.8, color='tab:grey')
ax.set_ylabel(r'$1-e_{\rm in}$')

ax=fig.add_subplot(413)
ax.plot(tt/1.e6/P_yr, theta1_SL*180./np.pi, alpha=0.8, label=r'$S_1$', color='tab:grey')
ax.plot(tt/1.e6/P_yr, theta2_SL*180./np.pi, alpha=0.8, label=r'$S_2$', color='tab:olive')
ax.axhline(90., color='tab:red', ls=':', alpha=0.5)
ax.set_ylabel(r'$\theta_{SL}$ [deg]')
ax.legend(loc='upper left')

ax=fig.add_subplot(414)
ax.plot(tt/1.e6/P_yr, theta_SS*180./np.pi, alpha=0.8, color='tab:grey')
ax.set_ylabel(r'$\theta_{SS}$ [deg]')
ax.set_xlabel(r'Time [$10^6$ yr]')
